function [val, min_reliability, len1, len2, len3, cost, contact, h, avg] = reliability_fun_3(supply, info0, info1, money, request_min_reliability, target, color, invoke_time)
% 计算双供网络，三条联络线情况下的可靠性
...
contact_line_3_re0 = contact_line_3_re0 * (1 - 0.002 * len3) * 0.998; % 联络线本身的可靠性
contact_line_3_re1 = contact_line_3_re1 * (1 - 0.002 * len3) * 0.998; % 联络线本身的可靠性
...
sum_reliability = zeros(1, size(nearest_dot0, 1) * size(nearest_dot1, 1) * size(nearest_dot0, 1) * size(nearest_dot1, 1));
min_reliability = zeros(1, size(nearest_dot0, 1) * size(nearest_dot1, 1) * size(nearest_dot0, 1) * size(nearest_dot1, 1));
[nd0, si0, po0] = reliability_fun0(info0{1}, supply(1, :), info0{3} );
[nd1, si1, po1] = reliability_fun0(info1{1}, supply(2, :), info1{3} );
len1 = 10000 * ones(1, size(nearest_dot0, 1) * size(nearest_dot1, 1) * size(nearest_dot0, 1) * size(nearest_dot1, 1) );
len2 = 10000 * ones(1, size(nearest_dot0, 1) * size(nearest_dot1, 1) * size(nearest_dot0, 1) * size(nearest_dot1, 1) );
    ...
    for j = 2:size(nearest_dot1, 1)
        for i2 = 2:size(nearest_dot0, 1)
            for j2 = 2:size(nearest_dot1, 1)
                ...
                contact_line2_length = dist(nearest_dot0(i2, :), nearest_dot1(j2, :)' );
                main_counter = (i-1) *  size(nearest_dot1, 1) * size(nearest_dot0, 1) * size(nearest_dot1, 1)...
                    + (j-1) * size(nearest_dot0, 1) * size(nearest_dot1, 1) + (i2 - 1) * size(nearest_dot1, 1) + j2;
                if 325.7 * (contact_line_length + contact_line2_length + len3) + 3 * 56.8 > money
                    continue;
                end
                ...
                for m = 1:size(info0{2}, 2) % 簇
                    for n = 1:size(info0{2}{m}, 1)
                        ...
                        elseif mod(j2, 2) == 1
                            contact_line2_re0 = reliability_fun00(1, nearest_dot1(j2, :), info1{2}, info1{3}, info1{4}, info1{5}, info1{6}, nd1, si1, po1);
                        else
                            contact_line2_re0 = reliability_fun00(1, nearest_dot1(j2+1, :), info1{2}, info1{3}, info1{4}, info1{5}, info1{6}, nd1, si1, po1);
                            contact_line2_re0 = contact_line2_re0 / (1 - 0.002 * 0.5 * length1(j2/2) );
                        end
                        ...
                        % 算上联络线本身的可靠性
                        contact_line2_re0 = contact_line2_re0 * 0.998 * (1 - 0.002 * contact_line2_length);
                        % 算上联络线到一级分叉点的可靠性
                        contact_line2_re0 = contact_line2_re0 * tmp_reliability;
                        ...
                        for o = tmp1 : size(nearest_dot0, 1)-1
                            tmp_len = tmp_len + 0.5 * length0(ceil(o / 2) );
                            if mod(o, 2) == 0
                                contact_line3_re0 = contact_line3_re0 * 0.998; % 开关
                                contact_line3_re0 = contact_line3_re0 * (1 - 0.002 * tmp_len);
                                tmp_len = 0;
                            end
                        end
                        ...
                        for e = 1:size(info0{2}, 2)
                            ...
                            if i2 >= 2 * (size(info0{2}, 2) - e) + 1 % 故障发生在联络线2之前，可靠性要计算联络线2
                                tmp_contact_re = tmp_contact_re * (1 - contact_line2_re0);
                            end
                ...
                for m = 1:size(info1{2}, 2)
                    for n = 1:size(info1{2}{m}, 1)
                        ...
                        elseif mod(i2, 2) == 1
                            contact_line2_re1 = reliability_fun00(1, nearest_dot0(i2, :), info0{2}, info0{3}, info0{4}, info0{5}, info0{6}, nd0, si0, po0);
                        else
                            contact_line2_re1 = reliability_fun00(1, nearest_dot0(i2+1, :), info0{2}, info0{3}, info0{4}, info0{5}, info0{6}, nd0, si0, po0);
                            contact_line2_re1 = contact_line2_re1 / (1 - 0.002 * 0.5 * length0(i2/2) );
                        end
                        ...
                        % 算上联络线本身的可靠性
                        contact_line2_re1 = contact_line2_re1 * 0.998 * (1 - 0.002 * contact_line2_length);
                        % 算上联络线到一级分叉点的可靠性
                        contact_line2_re1 = contact_line2_re1 * tmp_reliability;
                        ...
                        for o = tmp1 : size(nearest_dot0, 1)-1
                            tmp_len = tmp_len + 0.5 * length0(ceil(o / 2) );
                            if mod(o, 2) == 0
                                contact_line3_re1 = contact_line3_re1 * 0.998; % 开关
                                contact_line3_re1 = contact_line3_re1 * (1 - 0.002 * tmp_len);
                                tmp_len = 0;
                            end
                        end
                        ...
                        for e = 1:size(info1{2}, 2)
                            ...
                            if j2 >= 2 * (size(info1{2}, 2) - e) + 1 % 故障发生在联络线2之前，可靠性要计算联络线2
                                tmp_contact_re = tmp_contact_re * (1 - contact_line2_re1);
                            end
                ...
                len2(main_counter) = dist(nearest_dot0(i2, :), nearest_dot1(j2, :)' );
            end
        end
    end
end
...
if target == 3 % 第3问最低可靠性最大的方案
    if sum(min_reliability) == 0
        ...
        % 只连两条联络线
        disp("正在尝试只连两条联络线");
        [val, min_reliability, len1, len2, cost, contact, h, avg] = reliability_fun_2(supply, info0, info1, money, request_min_reliability, target, color, invoke_time+1);
        return;
    end
    while true
        ...
        max_index1 = ceil(max_index / (size(nearest_dot1, 1) *  size(nearest_dot0, 1) ) );
        max_index2 = mod(max_index, size(nearest_dot1, 1) *  size(nearest_dot0, 1) );
        % 得到方案对应的index
    ...
    contact = [row1, col1; row2, col2; size(nearest_dot0, 1), size(nearest_dot1, 1)];
    ...
    hold on;
    line([nearest_dot0(row2, 1), nearest_dot1(col2, 1)], [nearest_dot0(row2, 2), nearest_dot1(col2, 2)], 'Color', color, 'LineWidth', 4);
    hold on;
    line([nearest_dot0(size(nearest_dot0, 1), 1), nearest_dot1(size(nearest_dot1, 1), 1)], ...
        [nearest_dot0(size(nearest_dot0, 1), 2), nearest_dot1(size(nearest_dot1, 1), 2)], ...
...
elseif target == 4 % 第4问费用最低的方案
    ...
    while true 
        ...
        elseif min_reliability(min_index) < request_min_reliability % 如果不满足最低可靠性的要求
            ...
            % 得到方案对应的index
            min_index1 = ceil(min_index / (size(nearest_dot1, 1) *  size(nearest_dot0, 1) ) );
            min_index2 = mod(min_index, size(nearest_dot1, 1) *  size(nearest_dot0, 1) );

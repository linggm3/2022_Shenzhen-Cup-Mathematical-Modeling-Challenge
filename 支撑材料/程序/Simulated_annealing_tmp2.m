function [dot, cost, run_time, counter] = Simulated_annealing_tmp2(T, innerloop, dc, dot, supply, center, center_load, spacing, boxx, boxy, counter, ttt, aaa)
...
%         axis([-1.50, 2.50, -1.5, 2.5]);
        ...
        elseif aaa == 2
            scatter(center(:, 1), center(:, 2), 150, Color(2, :), 'd', "filled");
        end
        hold on;
        ...
        for ii = 1:cluster_num
            for j = 1:size(edge, 1)
                % 二级分叉点到总线每段的距离
                [d(1, j), nearest(j, :)] = distance_fun(center(ii, :), dott(edge(j, 1), :), dott(edge(j, 2), :) );
            end
            [~, pos] = min(d);
            res(ii, :) = nearest(pos, :);
            hold on;
        ...
        for i = 1:cluster_num % 对第一层的每个聚类中心
            ...
            for j = 1:size(edge, 1)
                % 二级分叉点到总线每段的距离
                [d(1, j), nearest(j, :)] = distance_fun(point(i, :), dott(edge(j, 1), :), dott(edge(j, 2), :) );
            end
            % 二级分叉点到总线的最短距离

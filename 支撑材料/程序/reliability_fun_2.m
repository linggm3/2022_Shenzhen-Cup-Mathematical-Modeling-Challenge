function [val, min_reliability, len1, len2, cost, contact, h, avg] = reliability_fun_2(supply, info0, info1, money, request_min_reliability, target, color, invoke_time)
% 计算双供网络，两条联络线情况下的可靠性
    ...
    for j = 2:size(nearest_dot1, 1)
        for i2 = 2:size(nearest_dot0, 1)
            for j2 = 2:size(nearest_dot1, 1)
                ...
                if 325.7 * (contact_line_length + contact_line2_length) + 2 * 56.8 > money
                    continue;
                end
...
if target == 3 % 第3问最低可靠性最大的方案
    if sum(min_reliability) == 0
        ...
        % 只连一条联络线
        disp("正在尝试只连一条联络线");
        [val, min_reliability, len1, cost, contact, h, avg] = reliability_fun_1(supply, info0, info1, money, request_min_reliability, target, color, invoke_time+1);
        len2 = -1;
        return;
    end
    while true
...
elseif target == 4 % 第4问费用最低的方案
    ...
    while true 
        ...
        if min_len == 20000 
            ...
            disp("两条联络线无法达到可靠性要求");
            disp("正在尝试连接三条联络线");
            [val, min_reliability, len1, len2, ~, cost, contact, h, avg] = reliability_fun_3(supply, info0, info1, money, request_min_reliability, target, color, invoke_time+1);
            return;

...
for time = 1:times % 双层规划次数
    ...
    if time == 1
        [best_individual, fit, run_time1, counter] = Genetic_algorithm_tmp(epoch, pop_size, individual_size, pm, supply, center, center_load, ttt);
...
%     [dot, fit, run_time1] = Genetic_algorithm(1000, pop_size, individual_size, pm, supply, center, center_load);
    ...
    [dot, cost, run_time2, counter] = Simulated_annealing_tmp2(temterature, innerloop, dc, dot, supply, center, center_load, spacing, boxx, boxy, counter, ttt, time);
...
%                     line([box_tmp(k, 1), center2{i}(j, 1)], [box_tmp(k, 2), center2{i}(j, 2)], 'Color', color(4, :), 'LineWidth', 1);
    ...
    total_cost(ttt) = cost_1 + cost_2 + cost_3 + cost_4 + cost_main;
